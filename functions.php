...
add_action('init', 'visionary_setup');
function visionary_setup()
{
...
// Add menu Support and removing the menu container.
	register_nav_menu('Main Menu', __('Your primary site menu','visionary'));
	register_nav_menu('Footer Menu', __('Your footer menu','visionary'));
...
function visionary_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'visionary_page_menu_args' );

/* Sets the excerpt length */
function visionary_excerpt_length( $length ) {
	return 40;
}
add_filter( 'excerpt_length', 'visionary_excerpt_length' );
...
// Changing excerpt ending to a more-link
   ...
   global $post;
   return '<a class="Logo-Text" href="'. get_permalink($post->ID) . '">' . __('Continue Reading','visionary') . '</a>';
   }
 ...
 */
function visionary_remove_recent_comments_style() {
	global $wp_widget_factory;
	remove_action( 'wp_head', array( $wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style' ) );
}
add_action( 'widgets_init', 'visionary_remove_recent_comments_style' );
...
function visionary_posted_on() {
	printf( __( '<span class="%1$s">Date: </span> %2$s <span class="meta-sep">&nbsp;Author: </span> %3$s', 'visionary' ),
		...
		sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
			...
			sprintf( esc_attr__( 'View all posts by %s', 'visionary' ), get_the_author() ),
			get_the_author()
		)
	);
}
...
if ( ! function_exists( 'visionary_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * To override this walker in a child theme without modifying the comments template
 * simply create your own visionary_comment(), and that function will be used instead.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 *
 * @since Twenty Ten 1.0
 */
function visionary_comment( $comment, $args, $depth ) {
	...
	<div <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<div class="commentgroup">
			<div id="comment-<?php comment_ID(); ?>">
				<div class="cmeta" colspan="2" valign="middle"><?php echo get_avatar( $comment, 40 ); ?><?php printf( __( '%s', 'visionary' ), sprintf( '<span class="cname">%s</span>', get_comment_author_link() ) ); ?><br />
	<span class="cdate"><?php _e('Commented','visionary'); ?>:&nbsp;<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
					<?php
						/* translators: 1: date, 2: time */
						printf( __( '%1$s at %2$s', 'visionary' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Edit)', 'visionary' ), ' ' );
					?>
					</span>
		</div>
  <?php if ( $comment->comment_approved == '0' ) : ?>
    <div colspan="2" class="cmoderation">
		<?php _e( 'Your comment is awaiting moderation.', 'visionary' ); ?></div>
<?php endif; ?>

    <div colspan="2" class="comment-body"><?php comment_text(); ?></div>
    <div colspan="2" class="reply"><?php comment_reply_link( array_merge( $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?></div>
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'visionary' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __('(Edit)', 'visionary'), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}
...
if ( ! function_exists( 'visionary_posted_in' ) ) :
/**
 * Prints HTML with meta information for the current post (category, tags and permalink).
 *
 * @since Twenty Ten 1.0
 */
function visionary_posted_in() {
	// Retrieves tag list of current post, separated by commas.
	$tag_list = get_the_tag_list( '', ', ' );
	if ( $tag_list ) {
		$posted_in = __( 'This entry was posted in %1$s and tagged %2$s. <br />Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'visionary' );
	} elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$posted_in = __( 'This entry was posted in %1$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'visionary' );
	} else {
		$posted_in = __( 'Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'visionary' );
	}
...
array( "name" => __('Logo and Title Settings', 'visionary' ),
	"type" => "section"),
array(  "type" => "open"),
		
	array(  "name" => __('Disable/Enable Default logo set', 'visionary' ),
		"desc" => __('Choose to enable or disable the default theme logo and title group for your own logo', 'visionary' ),
	...
	array(  
		"name" => __('Logo Image', 'visionary' ),
		"desc" => __('Add the link to your own logo image.', 'visionary' ),
		"id" => $shortname."_logo",
		"type" => "text",
		"std" => ""),
	
	array( 
		"name" => __('Logo ALT', 'visionary' ),
		"desc" => __('For seo benefits - enter your logo alt keywords like: The best WordPress themes', 'visionary' ),
		"id" => $shortname."_alt",
		"type" => "text",
		"std" => "WordPress themes by Pixel Theme Studio"),
			
array( "type" => "close"),		
array( "name" => __('Blog Settings', 'visionary' ),
	"type" => "section"),
array(  "type" => "open"),
		
	array(  "name" => __('Blog Left Column', 'visionary' ),
		"desc" => __('Choose to enable or disable the left column for your blog page. This is just for the blog only.', 'visionary' ),
		"id" => $shortname."_blogleft",
		"type" => "select",
		"std" => "Disable",
		"options" => array("Disable", "Enable")),
		
	array(  "name" => __('Blog Inset Column', 'visionary' ),
		"desc" => __('Choose to enable or disable the inset column for your blog page. This is just for the blog only.', 'visionary' ),
		"id" => $shortname."_bloginset",
		"type" => "select",
		"std" => "Disable",
		"options" => array("Disable", "Enable")),
		
	array(  "name" => __('Blog Right Column', 'visionary' ),
		"desc" => __('Choose to enable or disable the right column for your blog page. This is just for the blog only.', 'visionary' ),
		"id" => $shortname."_blogright",
		"type" => "select",
		"std" => "Enable",
		"options" => array("Enable", "Disable")),
		
	array(  "name" => __('Read More Button Style', 'visionary' ),
		"desc" => __('Choose the style of the read more button you want to use in your blog.', 'visionary' ),
		"id" => $shortname."_blogbuttonstyle",
		"type" => "select",
		"std" => "Logo-Text",
		"options" => array("Logo-Text", "Logo-Button", "Border-Button", "Light-Button")),
		
array( "type" => "close"),	
array(  "name" => __('Showcase and Slideshow Settings', 'visionary' ),
	"type" => "section"),
array(  "type" => "open"),
		
	array(  "name" => __('Home Page Showcase', 'visionary' ),
		"desc" => __('Choose either the Widget Showcase or the Content Slideshow that rotates images with a text intro to the right of the image.', 'visionary' ),
		"id" => $shortname."_sctype",
		"type" => "select",
		"std" => "Widget Showcase",
		"options" => array("Widget Showcase", "Content Slideshow")),
	
	array(  "name" => __('Showcase Left Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the left side of your showcase. Default colour is 594037', 'visionary' ),
		"id" => $shortname."_scleftbg",
		"type" => "text_colour",
		"std" => "594037"),
		
	array(  "name" => __('Showcase Background Colour', 'visionary' ),
		"desc" => __('Choose the background colour for your showcase area. Default colour is 1B2024', 'visionary' ),
		"id" => $shortname."_scbg",
		"type" => "text_colour",
		"std" => "1B2024"),
		
	array(  "name" => __('Showcase Right Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the right side of your showcase. Default colour is 6E5750', 'visionary' ),
		"id" => $shortname."_scrightbg",
		"type" => "text_colour",
		"std" => "6E5750"),
		
	array(  "name" => __('Disable/Enable Showcase Lines', 'visionary' ),
		"desc" => __('This disables or enables the line pattern that goes on top of the left and right sides of the Showcase areas.', 'visionary' ),
		"id" => $shortname."_sclines",
		"type" => "select",
		"std" => "Enable",
		"options" => array("Enable", "Disable")),
		
	array(  "name" => __('Disable/Enable Slideshow Intro', 'visionary' ),
		"desc" => __('Choose to enable or disable the Intro.', 'visionary' ),
		"id" => $shortname."_slideintro",
		"type" => "select",
		"std" => "Enable",
		"options" => array("Enable", "Disable")),
		
		
	array(  "name" => __('Slideshow Intro Background', 'visionary' ),
		"desc" => __('Choose the background colour of your slideshow intro. Default is 1B2024', 'visionary' ),
		"id" => $shortname."_introbg",
		"type" => "text_colour",
		"std" => "1B2024"),
		
	array(  "name" => __('Slideshow Title Colour', 'visionary' ),
		"desc" => __('Choose the colour of your intro title. Default is D4D4CB', 'visionary' ),
		"id" => $shortname."_introtitle",
		"type" => "text_colour",
		"std" => "D4D4CB"),
		
	array(  "name" => __('Slideshow Intro Text', 'visionary' ),
		"desc" => __('Choose the colour of your intro text. Default is B7B8B8', 'visionary' ),
		"id" => $shortname."_introtext",
		"type" => "text_colour",
		"std" => "B7B8B8"),
		
	array(  "name" => __('Slideshow Read More Button', 'visionary' ),
		"desc" => __('Choose to enable or disable the slideshow read more button', 'visionary' ),
		"id" => $shortname."_button",
		"type" => "select",
		"std" => "Enable",
		"options" => array("Enable", "Disable")),
		
	array(  "name" => __('Slideshow Button Style', 'visionary' ),
		"desc" => __('Choose the style of the read more button you want to use in your slideshow.', 'visionary' ),
		"id" => $shortname."_buttonstyle",
		"type" => "select",
		"std" => "Dark",
		"options" => array("Dark", "Border", "Logo")),
		
	array(  "name" => __('Slideshow Image Height', 'visionary' ),
		"desc" => __('Set your slideshow Image height (in Pixels)', 'visionary' ),
		"id" => $shortname."_imgheight",
		"type" => "text",
		"std" => "340"),
		
	array(  "name" => __('Slideshow Image Width', 'visionary' ),
		"desc" => __('Image width and the intro container width must equal 960 pixels. Default image width is 555 pixels.', 'visionary' ),
		"id" => $shortname."_imgwidth",
		"type" => "text",
		"std" => "555"),
			
	array(  "name" => __('Slideshow Intro Width', 'visionary' ),
		"desc" => __('Intro width plus image width must equal 960 pixels. Allow 50 pixels for intro padding on the left and right. Example with 555px image, your intro will be 405px - 50px equals 355px. Default intro width is 355 pixels.', 'visionary' ),
		"id" => $shortname."_introwidth",
		"type" => "text",
		"std" => "355"),
		
array( "type" => "close"),	
array(  "name" => __('Slideshow Content', 'visionary' ),
	"type" => "section"),
array(  "type" => "open"),

// slide 1
	array(  "name" => __('Disable/Enable Slide 1', 'visionary' ),
		"desc" => __('Choose to enable or disable Slide 1. Sliders should be 555x340 to fit correctly.', 'visionary' ),
		"id" => $shortname."_slide1",
		"type" => "select",
		"std" => "Enable",
		"options" => array("Enable", "Disable")),

	array(  "name" => __('Slide 1 Image', 'visionary' ),
		"desc" => __('Enter your image url (full path) here.', 'visionary' ),
		"id" => $shortname."_slide1img",
		"type" => "text",
		"std" => "#"),
		
	array(  "name" => __('Slide 1 Alt', 'visionary' ),
		"desc" => __('Enter your image description alt tag info here.', 'visionary' ),
		"id" => $shortname."_slide1alt",
		"type" => "text",
		"std" => __('The best theme ever', 'visionary' )),
		
	array(  "name" => __('Slide 1 Title', 'visionary' ),
		"desc" => __('Enter your slide title.', 'visionary' ),
		"id" => $shortname."_slide1title",
		"type" => "text",
		"std" => __('My First Title', 'visionary' )),
		
	array(  "name" => __('Slide 1 Intro', 'visionary' ),
		"desc" => __('Enter your slide intro text.', 'visionary' ),
		"id" => $shortname."_slide1text",
		"type" => "textarea",
		"std" => "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec mollis enim nec massa blandit rhoncus in gravida lasium justo eget velit vehicula quis eam."),
		
	array(  "name" => __('Slide 1 Button Text', 'visionary' ),
		"desc" => __('Enter your slide button text.', 'visionary' ),
		"id" => $shortname."_slide1btext",
		"type" => "select",
		"std" => "Read More",
		"options" => array(__('Read More', 'visionary' ), __('See More', 'visionary' ), __('Continue', 'visionary' ))),
	
	array(  "name" => __('Slide 1 Link', 'visionary' ),
		"desc" => __('Enter your slide read more URL (link).', 'visionary' ),
		"id" => $shortname."_slide1link",
		"type" => "text",
		"std" => "#"),

	// slide 2
	array(  "name" => __('Disable/Enable Slide 2', 'visionary' ),
		"desc" => __('Choose to enable or disable Slide 2', 'visionary' ),
		"id" => $shortname."_slide2",
		"type" => "select",
		"std" => "Disable",
		"options" => array("Disable", "Enable")),

	array(  "name" => __('Slide 2 Image', 'visionary' ),
		"desc" => __('Enter your image url (full path) here. Sliders should be 555x340 to fit correctly.', 'visionary' ),
		"id" => $shortname."_slide2img",
		"type" => "text",
		"std" => "#"),
		
	array(  "name" => __('Slide 2 Alt', 'visionary' ),
		"desc" => __('Enter your image description alt tag info here.', 'visionary' ),
		"id" => $shortname."_slide2alt",
		"type" => "text",
		"std" => __('The best theme ever', 'visionary' )),
		
	array(  "name" => __('Slide 2 Title', 'visionary' ),
		"desc" => __('Enter your slide title.', 'visionary' ),
		"id" => $shortname."_slide2title",
		"type" => "text",
		"std" => __('My First Title', 'visionary' )),
		
	array(  "name" => __('Slide 2 Intro', 'visionary' ),
		"desc" => __('Enter your slide intro text.', 'visionary' ),
		"id" => $shortname."_slide2text",
		"type" => "textarea",
		"std" => "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec mollis enim nec massa blandit rhoncus in gravida lasium justo eget velit vehicula quis eam."),
		
	array(  "name" => __('Slide 2 Button Text', 'visionary' ),
		"desc" => __('Enter your slide button text.', 'visionary' ),
		"id" => $shortname."_slide2btext",
		"type" => "select",
		"std" => __('Read More', 'visionary' ),
        "options" => array(__('Read More', 'visionary' ), __('See More', 'visionary' ), __('Continue', 'visionary' ))),
	
	array(  "name" => "Slide 2 Link",
		"desc" => "Enter your slide read more URL (link).",
		"id" => $shortname."_slide2link",
		"type" => "text",
		"std" => "#"),
		
// slide 3
	array(  "name" => __('Disable/Enable Slide 3', 'visionary' ),
		"desc" => __('Choose to enable or disable Slide 3', 'visionary' ),
		"id" => $shortname."_slide3",
		"type" => "select",
		"std" => "Disable",
		"options" => array("Disable", "Enable")),

	array(  "name" => __('Slide 3 Image', 'visionary' ),
		"desc" => __('Enter your image url (full path) here. Sliders should be 555x340 to fit correctly.', 'visionary' ),
		"id" => $shortname."_slide3img",
		"type" => "text",
		"std" => "#"),
		
	array(  "name" => __('Slide 3 Alt', 'visionary' ),
		"desc" => __('Enter your image description alt tag info here.', 'visionary' ),
		"id" => $shortname."_slide3alt",
		"type" => "text",
		"std" => __('The best theme ever', 'visionary' )),
		
	array(  "name" => __('Slide 3 Title', 'visionary' ),
		"desc" => __('Enter your slide title.', 'visionary' ),
		"id" => $shortname."_slide3title",
		"type" => "text",
		"std" => __('My First Title', 'visionary' )),
		
	array(  "name" => __('Slide 3 Intro', 'visionary' ),
		"desc" => __('Enter your slide intro text.', 'visionary' ),
		"id" => $shortname."_slide3text",
		"type" => "textarea",
		"std" => "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec mollis enim nec massa blandit rhoncus in gravida lasium justo eget velit vehicula quis eam."),
		
	array(  "name" => __('Slide 3 Button Text', 'visionary' ),
		"desc" => __('Enter your slide button text.', 'visionary' ),
		"id" => $shortname."_slide3btext",
		"type" => "select",
		"std" => "Read More",
        "options" => array(__('Read More', 'visionary' ), __('See More', 'visionary' ), __('Continue', 'visionary' ))),
	
	array(  "name" => __('Slide 3 Link', 'visionary' ),
		"desc" => __('Enter your slide read more URL (link).', 'visionary' ),
...
// slide 4
    array(  "name" => __('Disable/Enable Slide 4', 'visionary' ),
        "desc" => __('Choose to enable or disable Slide 4', 'visionary' ),
        "id" => $shortname."_slide4",
        "type" => "select",
        "std" => "Disable",
        "options" => array("Disable", "Enable")),

    array(  "name" => __('Slide 4 Image', 'visionary' ),
        "desc" => __('Enter your image url (full path) here. Sliders should be 555x340 to fit correctly.', 'visionary' ),
        "id" => $shortname."_slide4img",
        "type" => "text",
        "std" => "#"),

    array(  "name" => __('Slide 4 Alt', 'visionary' ),
        "desc" => __('Enter your image description alt tag info here.', 'visionary' ),
        "id" => $shortname."_slide4alt",
        "type" => "text",
        "std" => __('The best theme ever', 'visionary' )),

    array(  "name" => __('Slide 4 Title', 'visionary' ),
        "desc" => __('Enter your slide title.', 'visionary' ),
        "id" => $shortname."_slide4title",
        "type" => "text",
        "std" => __('My First Title', 'visionary' )),

    array(  "name" => __('Slide 4 Intro', 'visionary' ),
        "desc" => __('Enter your slide intro text.', 'visionary' ),
        "id" => $shortname."_slide4text",
        "type" => "textarea",
        "std" => "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec mollis enim nec massa blandit rhoncus in gravida lasium justo eget velit vehicula quis eam."),

    array(  "name" => __('Slide 4 Button Text', 'visionary' ),
        "desc" => __('Enter your slide button text.', 'visionary' ),
        "id" => $shortname."_slide4btext",
        "type" => "select",
        "std" => "Read More",
        "options" => array(__('Read More', 'visionary' ), __('See More', 'visionary' ), __('Continue', 'visionary' ))),

    array(  "name" => __('Slide 4 Link', 'visionary' ),
        "desc" => __('Enter your slide read more URL (link).', 'visionary' ),
        "id" => $shortname."_slide4link",
        "type" => "text",
        "std" => "#"),
		
// slide 5
    array(  "name" => __('Disable/Enable Slide 5', 'visionary' ),
        "desc" => __('Choose to enable or disable Slide 5', 'visionary' ),
        "id" => $shortname."_slide5",
        "type" => "select",
        "std" => "Disable",
        "options" => array("Disable", "Enable")),

    array(  "name" => __('Slide 5 Image', 'visionary' ),
        "desc" => __('Enter your image url (full path) here. Sliders should be 555x340 to fit correctly.', 'visionary' ),
        "id" => $shortname."_slide5img",
        "type" => "text",
        "std" => "#"),

    array(  "name" => __('Slide 5 Alt', 'visionary' ),
        "desc" => __('Enter your image description alt tag info here.', 'visionary' ),
        "id" => $shortname."_slide5alt",
        "type" => "text",
        "std" => __('The best theme ever', 'visionary' )),

    array(  "name" => __('Slide 5 Title', 'visionary' ),
        "desc" => __('Enter your slide title.', 'visionary' ),
        "id" => $shortname."_slide5title",
        "type" => "text",
        "std" => __('My First Title', 'visionary' )),

    array(  "name" => __('Slide 5 Intro', 'visionary' ),
        "desc" => __('Enter your slide intro text.', 'visionary' ),
        "id" => $shortname."_slide5text",
        "type" => "textarea",
        "std" => "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec mollis enim nec massa blandit rhoncus in gravida lasium justo eget velit vehicula quis eam."),

    array(  "name" => __('Slide 5 Button Text', 'visionary' ),
        "desc" => __('Enter your slide button text.', 'visionary' ),
        "id" => $shortname."_slide5btext",
        "type" => "select",
        "std" => "Read More",
        "options" => array(__('Read More', 'visionary' ), __('See More', 'visionary' ), __('Continue', 'visionary' ))),

    array(  "name" => __('Slide 5 Link', 'visionary' ),
        "desc" => __('Enter your slide read more URL (link).', 'visionary' ),
        "id" => $shortname."_slide5link",
        "type" => "text",
        "std" => "#"),
		
array( "type" => "close"),	
array(  "name" => __('Colour Settings', 'visionary' ),
	"type" => "section"),
array(  "type" => "open"),

	array(  "name" => __('Page Background', 'visionary' ),
		"desc" => __('Choose the background colour for the right side of your showcase. Default colour is C7C8CA', 'visionary' ),
		"id" => $shortname."_pagebg",
		"type" => "text_colour",
		"std" => "C7C8CA"),

	array(  "name" => __('Top Bar Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the top of your page. Default colour is E5D8C5', 'visionary' ),
		"id" => $shortname."_topbg",
		"type" => "text_colour",
		"std" => "E5D8C5"),
	
	array(  "name" => __('Header Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the header logo menu area. Default colour is FFFFFF', 'visionary' ),
		"id" => $shortname."_headerbg",
		"type" => "text_colour",
		"std" => "FFFFFF"),
		
	array(  "name" => __('Breadcrumb Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the breadcrumb navigation. Default colour is E8E8E8', 'visionary' ),
		"id" => $shortname."_breadcbg",
		"type" => "text_colour",
		"std" => "E8E8E8"),
		
	array(  "name" => __('Content Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the content area of your page. Default colour is F2F2F2', 'visionary' ),
		"id" => $shortname."_contentbg",
		"type" => "text_colour",
		"std" => "F2F2F2"),
	
	array(  "name" => __('Group 3 Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the Group 2 widget background. Default colour is E5D8C5', 'visionary' ),
		"id" => $shortname."_group3bg",
		"type" => "text_colour",
		"std" => "E5D8C5"),
		
	array(  "name" => __('Footer Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the footer background. Default colour is 8F979D', 'visionary' ),
		"id" => $shortname."_footerbg",
		"type" => "text_colour",
		"std" => "8F979D"),
		
	array(  "name" => __('Bottom Colour', 'visionary' ),
		"desc" => __('Choose the background colour for the bottom bar of your pages. Default colour is 6E5750', 'visionary' ),
		"id" => $shortname."_bottombg",
		"type" => "text_colour",
		"std" => "6E5750"),
	
	array(  "name" => __('Page Links Colour', 'visionary' ),
		"desc" => __('Choose the colour of your in-page text links', 'visionary' ),
		"id" => $shortname."_linkcolour",
		"type" => "text_colour",
		"std" => "704537"),
	
	array(  "name" => __('Page Link Hover', 'visionary' ),
		"desc" => __('Choose the colour of your in-page text link hover colour on mouseovers and active state', 'visionary' ),
		"id" => $shortname."_linkhcolour",
		"type" => "text_colour",
		"std" => "333"),
		
	array(  "name" => __('Main Menu Colour', 'visionary' ),
		"desc" => __('Choose the colour of your main menu items. Default colour is 6E693A', 'visionary' ),
		"id" => $shortname."_menucolour",
		"type" => "text_colour",
		"std" => "6E693A"),
		
	array(  "name" => __('Main Menu Hover', 'visionary' ),
		"desc" => __('Choose the colour of your main menu hover and active items', 'visionary' ),
		"id" => $shortname."_menuhcolour",
		"type" => "text_colour",
		"std" => "333"),

array( "type" => "close"),	

array(  "name" => __('Miscellaneous Settings', 'visionary' ),
	"type" => "section"),
array(  "type" => "open"),

	array(  "name" => __('Page Width', 'visionary' ),
		"desc" => __('Choose a full width layout or a short width up to 1200 pixels (both options has a fixed content width of 960 pixels).', 'visionary' ),
		"id" => $shortname."_width",
		"type" => "select",
		"std" => "Full",
		"options" => array("Full Width", "Boxed" )),
		
	array(  "name" => __('Disable/Enable Footer Menu', 'visionary' ),
		"desc" => __('Choose to enable or disable the footer menu', 'visionary' ),
		"id" => $shortname."_footermenu",
		"type" => "select",
		"std" => "Disable",
		"options" => array("Disable", "Enable")),
		
	array(  "name" => __('Copyright Information', 'visionary' ),
		"desc" => __('Enter your own copyright credit line.', 'visionary' ),
		"id" => $shortname."_copyright",
		"std" => "",
		"type" => "textarea"),

	array(  "name" => __('Contact Email', 'visionary' ),
		"desc" => __('Enter the email address you want to use for your contact form.', 'visionary' ),
		"id" => $shortname."_email",
		"std" => "my@email.com",
		"type" => "text"),
		
	array(  "name" => __('Google Analytics Code', 'visionary' ),
		"desc" => __('Enter your own Google Analytics code.', 'visionary' ),
...
if ( $_REQUEST['saved'] ) echo '<div id="message" class="updated fade" style="background:#87A85E; border-color:#4C693C; color:#fff; margin-left:5px;"><p><strong>'.$themename.' '.__('settings sucessfully saved.', 'visionary' ). '</strong><br><br><img src=""></p></div> ';
if ( $_REQUEST['reset'] ) echo '<div id="message" class="updated fade" style="background:#5992B5; border-color:#4A6A80; color:#fff; margin-left:5px;"><p><strong>'.$themename.' '.__('settings successfully reset.', 'visionary' ).'</strong></p></div>';
if ( $_REQUEST['error'] ) echo '<div id="message" class="updated fade" style="border-color:#733B2F; background:#B56D6B; color:#fff; margin-left:5px;"><p><strong>'. __('An error has occurred in the', 'visionary' ).' '.$themename.' '.__('theme', 'visionary' ).'</strong></p></div>';
...
<div id="logo"><img src='<?php echo $file_dir."/functions/images/logo.png"; ?>' alt="logo" /><h1> <?php echo $themename; ?> Theme Settings </h1></div>

<div class="m_help">
<p>
<strong><?php _e('Theme Support:', 'visionary' ); ?> </strong><?php _e('If you are experiencing difficulties with the', 'visionary' ); ?> <?php echo $themename; ?> <?php _e('theme, you can setup a membership with www.pixelthemestudio.ca if you do not have one. This gives you direct support in addition to the theme setup tutorials located at the site.', 'visionary' ); ?> </p>
</div>
...
case "title":
?>
<p><?php _e('To easily use the', 'visionary' ); ?> <?php echo $themename;?> <?php _e('theme, you can use the menu below.', 'visionary' ); ?></p>
...
function get_content_width(){
    $contentWidth = 960;
    if(get_option('pts_blogleft')!='Disable'){
        $contentWidth -= 250;
    }

    if(get_option('pts_bloginset')!='Disable'){
        $contentWidth -= 250;
    }

    if(get_option('pts_blogright')!='Disable'){
        $contentWidth -= 320;
    }
...
add_action( 'tgmpa_register', 'pts_register_required_plugins' );
function pts_register_required_plugins() {
    $plugins = array(
        array(
            'name'               => 'Visionary Shortcodes',
            'slug'               => 'visionary-shortcode',
            'source'             => 'visionary-shortcode.zip',
            'required'           => true,
            'version'            => '1.0.0',
        )
    );
    ...
    $config = array(
        ...
        'menu'         => 'transition-install-plugins',
        'has_notices'  => true,
        'dismissable'  => true,
        'dismiss_msg'  => '',
        'is_automatic' => false,
        ...
        'strings'      => array(
            'page_title'                      => __( 'Install Required Plugins', 'visionary' ),
            'menu_title'                      => __( 'Install Plugins', 'visionary' ),
            'installing'                      => __( 'Installing Plugin: %s', 'visionary' ),
            'oops'                            => __( 'Something went wrong with the plugin API.', 'visionary' ),
            ...
            'return'                          => __( 'Return to Required Plugins Installer', 'visionary' ),
            'plugin_activated'                => __( 'Plugin activated successfully.', 'visionary' ),
            'complete'                        => __( 'All plugins installed and activated successfully. %s', 'visionary' ),
            'nag_type'                        => 'updated'
        )
    );
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'menu_title'                     => __( 'Install Plugins', 'visionary' ),
                'installing'                     => __( 'Installing Plugin: %s', 'visionary' ),
                'oops'                           => __( 'Something went wrong.', 'visionary' ),
                ...
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'visionary' ),
                'dashboard'                      => __( 'Return to the dashboard', 'visionary' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'visionary' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'visionary' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'visionary' ),
                'dismiss'                        => __( 'Dismiss this notice', 'visionary' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'visionary' ) . '">' . __( 'Return to the Dashboard', 'visionary' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'eveningshade' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'eveningshade' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'eveningshade' ) . '</a>'  : '';
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'tgmpa', 'visionary', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'tgmpa', 'visionary', $rendered, $nag_class );
                }
            }
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'visionary' );
                        // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'visionary' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'visionary' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'visionary' ) : __( 'Recommended', 'visionary' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'visionary' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'visionary' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'visionary' ) . ' %2$s">' . __( 'Install', 'visionary' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'visionary' ) . ' %2$s">' . __( 'Activate', 'visionary' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'visionary' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'visionary' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'visionary' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'eveningshade' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'visionary' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>', 'visionary' ).'&#8230;';
                $this->strings['unpack_package']      = __( 'Unpacking the package', 'visionary' ).'&#8230;';
                $this->strings['installing_package']  = __( 'Installing the plugin', 'visionary' ).'&#8230;';
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'visionary' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'visionary' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'visionary' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'visionary' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'visionary' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'visionary' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'visionary' ) . '</span><span class="hidden">' . __( 'Hide Details', 'visionary' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'visionary' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'visionary' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'visionary' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'visionary' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'visionary' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'visionary' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'visionary' ) . '</span><span class="hidden">' . __( 'Hide Details', 'visionary' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'visionary' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'visionary' );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'visionary' ) . '">' . __( 'Return to the Dashboard', 'visionary' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
